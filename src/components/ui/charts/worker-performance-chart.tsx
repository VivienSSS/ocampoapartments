...
export function WorkerPerformanceChart() {
    const { data, isLoading, isError } = useQuery(workerPerformanceChartViewQuery());

    if (isLoading) {
        return (
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="text-base font-semibold">Worker Performance</CardTitle>
                </CardHeader>
                <CardContent>
                ...
                <ChartContainer config={chartConfig} className="h-[200px] w-full">
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={chartData}>
                            ...
                            <Bar dataKey="completedJobs" fill="var(--color-completedJobs)" radius={4} />
                            <Bar dataKey="inProgressJobs" fill="var(--color-inProgressJobs)" radius={4} />
                        </BarChart>
                    </ResponsiveContainer>
                </ChartContainer>
            </CardContent>
