...
export function OutstandingReceivablesChart() {
    const { data, isLoading, isError } = useQuery(outstandingReceivablesChartViewQuery());

    if (isLoading) {
        return (
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="text-base font-semibold">Outstanding Receivables</CardTitle>
                </CardHeader>
                <CardContent>
    ...
    return (
        <Card>
            <CardHeader className="pb-3">
                <CardTitle className="text-base font-semibold">Top Outstanding Receivables</CardTitle>
            </CardHeader>
            <CardContent>
                <ChartContainer config={chartConfig} className="h-[200px] w-full">
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={chartData}>
                            ...
                            <Bar dataKey="totalOutstanding" fill="var(--color-totalOutstanding)" radius={4} />
                        </BarChart>
                    </ResponsiveContainer>
                </ChartContainer>
            </CardContent>
