...
export function RevenuePerPropertyChart() {
    const { data, isLoading, isError } = useQuery(revenuePerPropertyChartViewQuery());

    if (isLoading) {
        return (
            <Card>
                <CardHeader className="pb-3">
                    <CardTitle className="text-base font-semibold">Revenue per Property</CardTitle>
                </CardHeader>
                <CardContent>
                ...
                <ChartContainer config={chartConfig} className="h-[200px] w-full">
                    <ResponsiveContainer width="100%" height={200}>
                        <BarChart data={chartData}>
                            ...
                            <Bar dataKey="currentMonthlyRevenue" fill="var(--color-currentMonthlyRevenue)" radius={4} />
                            <Bar dataKey="totalPotentialMonthlyRevenue" fill="var(--color-totalPotentialMonthlyRevenue)" radius={4} />
                        </BarChart>
                    </ResponsiveContainer>
                </ChartContainer>
            </CardContent>
