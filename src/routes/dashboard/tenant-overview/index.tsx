...
function RouteComponent() {
  ...
  const totalAnnouncementPages = Math.ceil(
    (announcements.items?.length ?? 0) / ANNOUNCEMENTS_PER_PAGE,
  );
  ...
  const paginatedAnnouncements =
    announcements.items?.slice(startIndex, endIndex) ?? [];
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Outstanding and recent bills</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {bills.items && bills.items.length > 0 ? (
              <div className="space-y-4">
                {bills.items
                  .filter((bill: BillsResponse) => bill.status !== 'Paid')
                  .slice(0, 5)
                  .map((bill: BillsResponse) => {
                    const tenantName = bill.expand?.tenancy?.expand?.tenant
                      ?.expand?.user
...
function BillCard({
  ...
  const totalAmount = (billItems?.items ?? []).reduce(
    (sum: number, item: any) => {
        ...
        <div className="flex justify-between items-start gap-4">
          <div className="flex-1">
            ...
            <div>
              ...
              <p className="font-semibold text-sm mt-1">{tenantName}</p>
