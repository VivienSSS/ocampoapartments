...
func init() {
	m.Register(func(app core.App) error {
		...
		if err := json.Unmarshal([]byte(`{
			...
			"listRule": "@request.auth.isActive = true && @request.auth.role = \"Administrator\" || (@request.auth.role = \"Tenant\" && @request.auth.id = tenancy.tenant.user.id)",
			"updateRule": "@request.auth.isActive = true && @request.auth.role = \"Administrator\"",
			"viewRule": "@request.auth.isActive = true && @request.auth.role = \"Administrator\" || (@request.auth.role = \"Tenant\" && @request.auth.id = tenancy.tenant.user.id)"
		}`), &collection); err != nil {
			return err
		}
