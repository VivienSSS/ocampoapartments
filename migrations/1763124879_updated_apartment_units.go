...
func init() {
	m.Register(func(app core.App) error {
		...
		if err := json.Unmarshal([]byte(`{
			...
			"listRule": "(@request.auth.id = \"\" && isAvailable = true) || @request.auth.id != \"\" ||\n@request.auth.role = \"Administrator\" || @request.auth.role = \"Building Admin\"",
			"updateRule": "@request.auth.role = \"Administrator\" || @request.auth.role = \"Building Admin\"",
			"viewRule": "(@request.auth.id = \"\" && isAvailable = true) || @request.auth.id != \"\" || @request.auth.role = \"Administrator\" || @request.auth.role = \"Building Admin\""
		}`), &collection); err != nil {
			return err
		}
