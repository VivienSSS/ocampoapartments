...
async function seedProperties(count: number = 2) {
    ...
    for (let i = 0; i < count; i++) {
        const property = await pb.collection(Collections.Properties).create({
            branch: faker.helpers.arrayElement(Object.values(PropertiesBranchOptions)),
...
async function seedApartments(properties: PropertiesResponse[], unitsPerProperty: number = 5) {
...
async function seedTenancies(tenants: TenantsResponse[], apartments: ApartmentUnitsResponse[]) {
    ...
    for (const tenant of tenants) {
        ...
        const tenancy = await pb.collection(Collections.Tenancies).create({
            ...
            leaseEndDate: faker.date.future({ years: 1, refDate: startDate }).toISOString(),
        });
...
async function seedMaintenanceRequests(tenancies: TenanciesResponse[], workers: MaintenanceWorkersResponse[], count: number = 15) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const request = await pb.collection(Collections.MaintenanceRequests).create({
            tenant: tenancy.tenant,
            unit: tenancy.unit,
            worker: faker.helpers.arrayElement(workers).id,
            description: faker.lorem.paragraph(),
            urgency: faker.helpers.arrayElement(Object.values(MaintenanceRequestsUrgencyOptions)),
            status: faker.helpers.arrayElement(Object.values(MaintenanceRequestsStatusOptions)),
            submittedDate: submittedDate.toISOString(),
            completedDate: faker.date.future({ years: 0.1, refDate: submittedDate }).toISOString(),
        });
...
async function seedBills(tenancies: TenanciesResponse[], count: number = 30) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const status = faker.helpers.arrayElement(Object.values(BillsStatusOptions));
        let totalAmount = 0;
        ...
        for (let j = 0; j < itemCount; j++) {
            ...
            await pb.collection(Collections.BillItems).create({
                bill: bill.id,
                chargeType: faker.helpers.arrayElement(Object.values(BillItemsChargeTypeOptions)),
                amount: amount,
...
async function seedPayments(bills: BillsResponse[], tenants: TenantsResponse[]) {
    ...
    for (const bill of bills) {
        ...
        if (bill.status === BillsStatusOptions.Paid) {
            try {
                ...
                const billDetails = await pb.collection(Collections.Bills).getOne(bill.id, {
                    expand: 'tenancy.tenant'
                });

                // Get the tenant from the expanded data or pick a random one
                const tenantId = billDetails.expand?.tenancy?.tenant?.id || faker.helpers.arrayElement(tenants).id;
                ...
                const payment = await pb.collection(Collections.Payments).create({
                    ...
                    paymentMethod: faker.helpers.arrayElement(Object.values(PaymentsPaymentMethodOptions)),
